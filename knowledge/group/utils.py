    ...
    if layer == '2':  #扩展两层
        ...
        for uid_value in uid_list:
            ...
            for i in list(result):
                ...
                if end_id.has_key('uid'):
                    nodes_list.append(end_id['uid'])
                    user_relation.append([start_id,relation1,middle_id,relation2,end_id['uid']])
                if end_id.has_key('envent_id'):
                    nodes_list.append(end_id['uid'])
                    user_relation.append([start_id,relation1,middle_id,relation2,end_id['envent_id']])
                    # user_relation.append([start_id,relation,end_id['envent_id']])
                # print i['s1'].labels()
                # print i['r'].type()#输出关系类型
                # print dict(i['s1'])#输出节点的属性
    return {'total_user':len(uid_list),'nodes':nodes_list, 'relation':user_relation}
...
def group_tab_map(group_name, node_type, relation_type, layer):
    ...
    # results = es_event.search(index=event_analysis_name, doc_type = event_type, body=query_node )
    results = es_user_portrait.search(index=portrait_name, doc_type=portrait_type, \
                                    body=query_node)["aggregations"]["all_location"]["buckets"]
