...
function tupu() {
    ...
    function nums() {
        ...
        url='/theme/t_compare_graph/?theme_name1='+themename1+'&theme_name2='+themename2+'&layer='+
            point+'&diff='+numberd;
        url2='/theme/t_compare_map/?theme_name1='+themename1+'&theme_name2='+themename2+'&layer='+
            point+'&diff='+numberd;
    ...
    function events() {
        ...
        $.getJSON(url, function (json) {
            //图表数据
            if (!json.e1.node.event_id){
                $('#eventimg').html('<p style="text-align:center;height: 500px;line-height: 500px">无数据更新</p>');
                $('#container #middle .centre .midleft .conevents .peonum').html(0);
            }else {
                var json=eval(json);
                ...
                for (var key2 in json.e1.node.uid){
                    ...
                    var name2;
                    if (json.e1.node.uid[key2]==''||json.e1.node.uid[key2]=="unknown") {
                        name2=key2;
                    }else {
                        name2=json.e1.node.uid[key2];
                    }
                    node_value.push(
                        {
                            x: num3,
        ...
        $.getJSON(url, function (json) {
            if (!json.e2.node.event_id){
                $("#placeimg").html('<p style="text-align:center;height: 500px;line-height: 500px">无数据更新</p>');
                $('#container #middle .centre .midright .placet .peonum').html(0);
            }else {
                var json=eval(json);
                ...
                for (var key2 in json.e2.node.uid){
                    ...
                    var num4=Math.random()*(-1000-700)+1000;
                    var name2;
                    if (json.e2.node.uid[key2]==''||json.e2.node.uid[key2]=="unknown") {
                        name2=key2;
                    }else {
                        name2=json.e2.node.uid[key2];
                    }
                    node_value2.push(
                        {
                            x: num3,
    ...
    function ditu() {
        ...
        $.getJSON(url2, function (json2) {
            ...
            var data = [];
            if (data2.e1==''){
                $("#eventimg2").html('<p style="text-align:center;height: 500px;line-height: 500px">无数据更新</p>');
            }else {
                $.each(data2.e1, function (index, item) {
                    data.push(
        ...
        $.getJSON(url2, function (json3) {
            ...
            var data = [];
            if (data3.e2==''){
                $("#placeimg2").html('<p style="text-align:center;height: 500px;line-height: 500px">无数据更新</p>');
            }else {
                $.each(data3.e2, function (index, item) {
                    data.push(
                        {name: item[0],value: item[1]}
                    );
                });
