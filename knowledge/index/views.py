...
from utils import query_current_week_increase, query_special_event, query_group,filter_event_map,\
     ...
     search_related_e_card, advance_search_card, advance_search_graph
...
def search_advance_user():  #高级搜索，卡片和图谱
    ...
    rel_type = request.args.get('rel_type', '')  #字符串，用逗号人物之间的把关系串联起来
    graph_info = advance_search_graph(id_list, layer, rel_type)
...
def search_advance_event():  #高级搜索，事件卡片和图谱
    ...
    tag_items = request.args.get('tag', '')
    query.append({'range':{'start_ts':{'gte':start_ts, 'lt':end_ts}}})
    print query,'---------'
